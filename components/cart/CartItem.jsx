...
import { setDecreaseItemQTY, setIncreaseItemQTY, setRemoveItemFromCart } from "../../contexts/CartSlice.js";

const CartItem = ({ item: { id, nombre, text, img, color, shadow, precio, cartQuantity } }) => {
      ...
      <div className="flex items-center justify-between w-full px-5">
        <div className="flex items-center gap-5">
          <div className={`bg-gradient-to-b ${color} ${shadow} relative rounded p-3 hover:scale-105 transition-all duration-75 ease-in-out grid items-center`}>
            <img src={img} alt={`img/cart-item/${id}`} className="w-36 h-24 object-fill lg:w-28" />
            {/* <div className='absolute right-1 top-1 blur-theme-effect bg-white/80 text-black text-xs px-1 rounded'>${precio}</div> */}
          </div>
          <div className="grid items-center gap-4">
            <div className="grid items-center leading-none">
              <h1 className="font-medium text-lg text-slate-900 lg:text-sm">{nombre}</h1>
        ...
        <div className="grid items-center gap-5">
          <div className="grid items-center justify-center">
            <h1 className="text-lg lg:text-base text-slate-900 font-medium">${precio * cartQuantity}</h1>
          </div>
          <div className="grid items-center justify-center">
